...
def __lldb_init_module(debugger, internal_dict):
    ...
    'command script add -f dtrace.handle_command dtrace -h "Generates DTrace profiling scripts"')
...
def handle_command(debugger, command, exe_ctx, result, internal_dict):
    ...
    stderr = "| grep -E 'ServiceBus' | grep -E '^\S*$'" if not options.debug_with_clipboard else ''
...
def generateDTraceScript(options):  
    ...
    script += "#!/usr/sbin/dtrace -s\n"
    script += "#pragma D option quiet\n"
    script += 'dtrace:::BEGIN { printf("Starting...\\n"); }\n'
    script += 'dtrace:::END   { printf("Ending...\\n"  ); }\n'
    # script += 'objc$target:::entry { printf("%s\\n", probefunc); }\n'
    script += 'pid$target:Filtrete::entry {  printf("%s\\n", probefunc); }\n'
